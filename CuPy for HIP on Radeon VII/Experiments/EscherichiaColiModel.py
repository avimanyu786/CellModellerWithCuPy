...
from CellModeller.Biophysics.BacterialModels.CLBacterium2CuPyTestWithDoublePrecision import CLBacterium2CuPyTestWithDoublePrecision
...
def setup(sim):
    ...
    biophys = CLBacterium2CuPyTestWithDoublePrecision(sim, jitter_z=True, rho=1.105, u=0.03, gammacoeff=0.59, max_planes=1)
...
def init(cell):
    ...
    ecoli_diameter = numpy.random.uniform(0.25, 1.0)
    cell_lens = {1: numpy.random.uniform(1.5, 3.5)}
    cellVol = numpy.add(numpy.divide ( numpy.multiply ( numpy.pi, numpy.multiply ( numpy.square (ecoli_diameter) , cell_lens[cell.cellType] ) ) , 4.0 ) , numpy.divide( numpy.multiply ( math.pi, numpy.power(ecoli_diameter , 3)) , 6.0 ) )

    cell.targetVol = numpy.add ( cellVol , numpy.random.uniform (0.0, 1.5) )
...
def update(cells):
    ...
    for (id, cell) in cells.iteritems():
        # Variable growth rates from 1.0 to 12.0
        cell.growthRate = numpy.random.uniform(0.0, cell_growr[cell.cellType])

        cell.color = [1.0, numpy.random.uniform(0.5, 0.7) , 1.0]
...
def divide(parent, d1, d2):
    ...
    ecoli_diameter = numpy.random.uniform(0.25, 1.0)
    cell_lens = {1: numpy.random.uniform(1.5, 3.5)}
    d1.targetVol = numpy.add ( numpy.add(numpy.divide(numpy.multiply(math.pi, numpy.multiply(numpy.square(ecoli_diameter), cell_lens[parent.cellType])), 4.0), numpy.divide(numpy.multiply(math.pi, numpy.power(ecoli_diameter, 3)), 6.0)) , numpy.random.uniform(0.0,1.5))

    ecoli_diameter = numpy.random.uniform(0.25, 1.0)
    cell_lens = {1: numpy.random.uniform(1.5, 3.5)}
    d2.targetVol = numpy.add ( numpy.add(numpy.divide(numpy.multiply(math.pi, numpy.multiply(numpy.square(ecoli_diameter), cell_lens[parent.cellType])), 4.0), numpy.divide(numpy.multiply(math.pi, numpy.power(ecoli_diameter, 3)), 6.0)), numpy.random.uniform(0.0, 1.5))
