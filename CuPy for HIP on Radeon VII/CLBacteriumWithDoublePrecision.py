...
class CLBacteriumWithDoublePrecision:
    ...
    def __init__(self, simulator,
        ...
        self.muA = muA
        self.gamma = numpy.double((rho * u * u * gammacoeff * refarea) / 2)
        self.dt = dt
        self.cgs_tol = cgs_tol
        self.reg_param = numpy.double(reg_param)
    ...
    def matrixTest(self):
        ...
        for i in range(self.n_cells):
            x = numpy.zeros((self.n_cells,), vec.double8)
            for j in range(7):
                if j>0:
                    x[i][j-1]=0.0
                x[i][j]=1.0
    ...
    def progress_init(self, dt):
        ...
        self.actual_dt = dt / numpy.double(self.n_ticks)
    ...
    def bin_cells(self):
        ...
        self.program.bin_cells(self.queue,
                               ...
                               numpy.double(self.grid_spacing),
    ...
    def build_matrix(self):
        ...
        self.program.build_matrix(self.queue,
                                  ...
                                  None,
                                  numpy.int32(self.max_contacts),
                                  numpy.double(self.muA),
                                  numpy.double(self.gamma),
    ...
    def CGSSolve(self, dt, substep=False):
        ...
        for iter in range(max_iters):
            ...
            # p = res + rsnew/rsold *p
            self.vaddkx(self.p_dev[0:self.n_cells], numpy.double(rsnew/rsold), self.res_dev[0:self.n_cells], self.p_dev[0:self.n_cells])
