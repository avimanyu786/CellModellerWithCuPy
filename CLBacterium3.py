...
class CLBacterium3:
    ...
    def init_kernels(self):
        ...
        kernel_src = resource_string(__name__, 'CLBacterium3.cl')
    ...
    def progress_finalise(self):
        ...
        self.sphDiaM = numpy.float32(self.sphDiaC * math.sqrt(2 * self.n_cells))
        if self.frame_no % 10 == 0:
            ...
            print '\nApproximate diameter of current microcolony = %f micrometre' % (self.sphDiaM)
        # pull cells from the device and update simulator
        if self.simulator:
            self.get_cells()
            for state in self.simulator.cellStates.values():
...
if __name__ == '__main__':
    ...
    model = CLBacterium3(sim, max_cells=2 ** 15, max_contacts=32, max_sqs=64 * 16, jitter_z=False, reg_param=2,
                         gamma=5.0)
    model.addPlane((0, -16, 0), (0, 1, 0), 1)
    model.addPlane((0, 16, 0), (0, -1, 0), 1)
    # model = CLBacterium3(None)
