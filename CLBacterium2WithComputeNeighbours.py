...
class CLBacterium2WithComputeNeighbours:
    """A rigid body model of bacterial growth implemented using
    OpenCL. Here, gamma, the frictional drag has been
    broken down into mass density, flow velocity, frictional drag
    coefficient and reference area.
    """
    ...
    def __init__(self, simulator,
        ...
        # Using both cupy.zeros with dtype numpy.int64 for cell_cts in updateCellNeighbours()
    ...
    def load_1024_cells(self):
        ...
        for i in range(-d / 2, d / 2):
            for j in range(-d / 2, d / 2):
                n = (i + d / 2) * d + (j + d / 2)
                x = i * 3.5 + numpy.random.uniform(-0.05, 0.05)
                y = j * 2.0 + numpy.random.uniform(-0.05, 0.05)
                th = numpy.random.uniform(-0.15, 0.15)
                dir_x = numpy.cos(th)
                dir_y = numpy.sin(th)
    ...
    def progress_init(self, dt):
        ...
        self.actual_dt = dt / numpy.float(self.n_ticks)
