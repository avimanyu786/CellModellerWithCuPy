...
#cell_lens = {0: numpy.random.uniform(0.8,1.0)}
#Maximum growth rate
cell_growr = {0: 12.0}
...
def setup(sim):
    ...
    #biophys = CLBacterium(sim, jitter_z=True, gamma=0.943666, max_planes=1, max_cells=100000)
    #biophys = CLBacteriumWithDoublePrecision(sim, jitter_z=True, gamma=0.943666, max_planes=1, max_cells=100000)
    biophys = CLBacteriumWithDoublePrecision(sim, jitter_z=True, rho=1.0039, u=0.02, gammacoeff=0.47, max_planes=1, max_cells=20000)
...
def init(cell):
    ...
    cell_lens = {0: numpy.random.uniform(0.8, 1.0)}
    cellVol = numpy.divide ( numpy.multiply ( numpy.power ( cell_lens[cell.cellType] , 3 ) , numpy.pi ) , 6.0 )

    cell.targetVol = numpy.add ( cellVol , numpy.random.uniform (0.0, 1.5) )
    #cell.targetVol = cellVol + numpy.random.uniform (0.0, 1.5)
    ...
    #cell.growthRate = numpy.random.uniform(1.0,cell_growr[cell.cellType])
...
def divide(parent, d1, d2):
    ...
    #d1.targetVol = (((cell_lens[parent.cellType]**3)*3.14) / 6) + numpy.random.uniform(0.0,1.5)
    #d2.targetVol = (((cell_lens[parent.cellType]**3)*3.14) / 6) + numpy.random.uniform(0.0,1.5)
    cell_lens = {0: numpy.random.uniform(0.8, 1.0)}
    d1.targetVol = numpy.add ( numpy.divide ( numpy.multiply ( numpy.power ( cell_lens[parent.cellType] , 3 ) , numpy.pi ) , 6.0 ) , numpy.random.uniform (0.0 , 1.5) )
    cell_lens = {0: numpy.random.uniform(0.8, 1.0)}
    d2.targetVol = numpy.add ( numpy.divide ( numpy.multiply ( numpy.power ( cell_lens[parent.cellType] , 3 ) , numpy.pi ) , 6.0 ) , numpy.random.uniform (0.0 , 1.5) )
